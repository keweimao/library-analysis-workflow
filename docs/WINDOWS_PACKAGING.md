...
The build runs on a native Windows x64 CI runner. It checks that the frozen executable serves `/api/health`, `/api/state`, and `/`, and can read the bundled analysis fingerprint used in result provenance. A separate Windows acceptance test runs the packaged install command, downloads the standalone Ollama archive, starts it, pulls a small model, then runs the packaged start command and checks the server again. These tests do not measure latency on every user's hardware. The package is unsigned; Windows SmartScreen or institutional policy may require approval. The app is single-user and local-only, not a public web-server deployment.
